...
def run_xgb(
    ...
    labels = np.array(
        [two4two.scene_parameters.OBJ_NAME_TO_INT[name] for name in df["obj_name"]]
    )
    ...
    return XGBResult(
        ...
        feature_importance=dict(
            zip(df_feat.keys(), map(float, gbm.feature_importances_))
        ),
...
def render_dataset_split(args: RenderSplitArgs):
    ...
    if args.unbiased:
        original_params = [
            sampler.sample_unbiased() for _ in tqdm.trange(args.n_samples)
        ]
    else:
        original_params = [sampler.sample() for _ in tqdm.trange(args.n_samples)]

    original_args = args.get_original()
    params_per_split = {original_args.get_key(): (original_args, original_params)}
...
def load_configs(
    ...
    for config in dataset_configs["dataset"]:
        ...
        for dset_args in config["split"]:
            ...
            interventions = tuple(
                tuple(intervention) for intervention in interventions_list
            )

            split_interventions = tuple(
                get_arg("split_interventions", split_interventions)
            )
